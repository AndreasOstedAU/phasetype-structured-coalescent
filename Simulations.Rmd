...
loss_function <- function(X) {
  ...
  S <- generate_subint_matrix(n, D, M = matrix(rep(m1, D*D), nrow = D, ncol = D))$subint_mat 
  reward_vector = head(rowSums(state_space(n,D)), -1)
  

  # Formula (4.7) in Wakeley, 2009 using phase-type moments
  mean_t_total <- ( alpha %*% solve(-S) %*% diag(reward_vector) %*% matrix(rep(1, length(alpha)), nrow = length(alpha)))
  
  mean_S = (theta/2)*mean_t_total
  
  # Fomula (4.8) in Wakeley, 2009 using phase-type moments
  second_moment <- (2*alpha %*% solve(((-S)%^%2)) %*% (diag(reward_vector) %^% 2)) %*% matrix(rep(1, length(alpha)), nrow = length(alpha))
  
  var_S = mean_S + (theta/2)^2 *( second_moment - mean_t_total^2)
...
S <- generate_subint_matrix(n, D, M = matrix(rep(1, D*D), nrow = D, ncol = D))$subint_mat #m is 1 by default
...
loss_mle <- function(X) {
  ...
  evo_model <- SFS_structured(theta=theta,n=n,D=D,within=within, m=1)
